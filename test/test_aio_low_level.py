...
async def test_standard_interfaces():
    async with MessageBus().connect() as bus:
        ...
        assert reply.signature == Array[Str].tree.signature
...
async def test_sending_signals_between_buses():
    async with MessageBus().connect() as bus1, \
        ...
        await bus2.send(
            Message.new_signal('/org/test/path', 'org.test.interface', 'SomeSignal', Str,
                               ['a signal']))
        ...
        assert signal.signature == Str.tree.signature
